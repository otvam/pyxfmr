...
def get_steinmetz():
    ...
    with open(os.path.join(folder, "transformer_data", "param_steinmetz.json")) as fid:
        data = json.load(fid)
...
def get_shape(geom):
    ...
    # load the values
    folder = os.path.dirname(os.path.abspath(__file__))
    with open(os.path.join(folder, "transformer_data", "param_shape.json")) as fid:
        data = json.load(fid)
...
def get_waveform(conv, phase):
    ...
    # load the values
    folder = os.path.dirname(os.path.abspath(__file__))
    with open(os.path.join(folder, "transformer_data", "param_waveform.json")) as fid:
        data = json.load(fid)
...
def get_design(geom, split, conv, simplified):
    ...
    #   - 1x1p - single-phase converter / one single-phase transformer
    #   - 1x3p - three-phase converter / one three-phase transformer (wye or delta connections)
    #   - 3x1p - three-phase converter / three single-phase transformers (wye or delta connections)
    if split == "1x1p":
